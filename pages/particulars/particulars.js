  ...
  sellInfoDetails: function() {
    ...
    util.sellInfoDetails(param, function(ret) {
      ...
      that.data.message_details.push({
        ...
        details: ret.content, //信息详情描述
        introduce: ret.introduce,//信息描述简介
...
//网站导入信息需副文本处理
  ...
  drawCanvas: function() {
    ...
    canvas.fillText(that.data.message_details[0].name, windowW * 0.23, windowH * 0.24);
    canvas.setFillStyle('#999999');
    canvas.setFontSize(24);
    canvas.fillText(that.data.message_details[0].position, windowW * 0.23 + canvas.measureText(that.data.message_details[0].name).width + 40, windowH * 0.24);
    // 供应 求购 纺机 背景颜色
    console.log('距离', canvas.measureText(that.data.message_details[0].demand).width)

    if (that.data.message_details[0].demand == '供应') {
      canvas.setFillStyle('#FFD634');
      canvas.fillRect(windowW * 0.55, windowH * 0.172, 50, 32);
      canvas.setFillStyle('#ffffff');
      canvas.setFontSize(20);
      canvas.fillText(that.data.message_details[0].demand, windowW * 0.555, windowH * 0.232);
    }
    if (that.data.message_details[0].demand == '求购') {
    ...
    if (that.data.message_details[0].demand == '纺机') {
    ...
    canvas.fillText(that.data.message_details[0].company, windowW * 0.23, windowH * 0.38);

    //标签
    if (that.data.message_details[0].lableList[0].tagname != '') {
      canvas.setFillStyle('#999999');
      canvas.setFontSize(28);
      canvas.fillText('#' + that.data.message_details[0].lableList[0].tagname + '#', windowW * 0.07, windowH * 0.55);
    }
    if (that.data.message_details[0].lableList.length >= 2) {
      canvas.setFillStyle('#999999');
      canvas.setFontSize(28);
      canvas.fillText('#' + that.data.message_details[0].lableList[1].tagname + '#', windowW * 0.15 + canvas.measureText(that.data.message_details[0].lableList[0].tagname).width, windowH * 0.55);
    }
    if (that.data.message_details[0].lableList.length >= 3) {
      canvas.setFillStyle('#999999');
      canvas.setFontSize(28);
      canvas.fillText('#' + that.data.message_details[0].lableList[2].tagname + '#', windowW * 0.23 + canvas.measureText(that.data.message_details[0].lableList[1].tagname).width + canvas.measureText(that.data.message_details[0].lableList[0].tagname).width, windowH * 0.55);
    }

    //描述
    var text = that.data.message_details[0].introduce; //这是要绘制的文本
